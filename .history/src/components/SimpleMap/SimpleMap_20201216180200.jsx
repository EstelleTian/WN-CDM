 ...
 * @LastEditTime: 2020-12-16 18:02:00
...
class SimpleMap extends React.Component {
     ...
     * @param zoomIndex
     */
    layerShowRule(layer, zoomIndex, mainMap) {
        //排除航路和航路点和经纬网图层
     ...
     * @param obj 图层对象
     */
    controlMapsFunc(mainMap, obj) {
        //获取当前缩放层级
        var zoomIndex = mainMap.getZoom();
        if (typeof obj == "array") {
            //遍历每个图层
            obj.map(function (index, e) {
                this.layerShowRule(e, zoomIndex, mainMap)
            });
        } else {
            // 单图层
            this.layerShowRule(obj, zoomIndex, mainMap)
        }
    };
    setMap(){
        ...
        var layersArr = [];
        //绑定到图层添加事件控制title显示
        for(let index in layersGroup ){
            let aipLayer = layersGroup[index]
            //显示事件
            aipLayer.on("add", function () {
                this.controlMapsFunc(mainMap, aipLayer)
            });
            //缩放事件
        ...
        mainMap.on("zoomend", function () {
            this.controlMapsFunc(mainMap, layersArr)
        })
